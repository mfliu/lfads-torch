...
def compute_l2_penalty(lfads, hps):
    ...
    for dIdx in range(0, len(lfads.decoders)):
        decoder = lfads.decoders[dIdx]
        recurrent_kernels_and_weights.append(
            (decoder.rnn.cell.gen_cell.weight_hh, hps.l2_gen_scale[dIdx]))
        if lfads.use_con[dIdx]:
            recurrent_kernels_and_weights.extend(
                [
                    (lfads.encoder.ci_enc[dIdx].fwd_gru.cell.weight_hh, hps.l2_ci_enc_scale[dIdx]),
                    (lfads.encoder.ci_enc[dIdx].bwd_gru.cell.weight_hh, hps.l2_ci_enc_scale[dIdx]),
                    (decoder.rnn.cell.con_cell.weight_hh, hps.l2_con_scale[dIdx]),
                ]
            )
