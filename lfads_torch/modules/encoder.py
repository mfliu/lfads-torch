...
class Encoder(nn.Module):
    def __init__(self, hparams: dict):
        ...
        self.use_con = [all([hps.ci_enc_dim[x] > 0, hps.con_dim[x] > 0, hps.co_dim[x] > 0]) for x in range(0, len(hps.ci_enc_dim))]
        self.ci_enc_h0 = []
        self.ci_enc = []
        for c_idx in range(0, len(self.use_con)):
            con = self.use_con[c_idx]
            if con:
                # Initial hidden state for CI encoder
                ci_enc_h0 = nn.Parameter(
                    torch.zeros((2, 1, hps.ci_enc_dim[c_idx]), requires_grad=True)
                )
                # CI encoder
                ci_enc = BidirectionalClippedGRU(
                    input_size=hps.encod_data_dim,
                    hidden_size=hps.ci_enc_dim[c_idx],
                    clip_value=hps.cell_clip,
                )
                self.ci_enc_h0.append(ci_enc_h0)
                self.ci_enc.append(ci_enc)
            else:
                self.ci_enc_h0.append(None)
                self.ci_enc.append(None)
    ...
    def forward(self, data: torch.Tensor):
        ...
        for c_idx in range(0, len(self.use_con)):
            ...
            if con == True:
                ...
                ci_enc_h0 = torch.tile(self.ci_enc_h0[c_idx], (1, batch_size, 1)).to(ci_enc_data.get_device())
                ci_enc = self.ci_enc[c_idx].to(ci_enc_h0.get_device())
                ci, _ = ci_enc(ci_enc_data, ci_enc_h0)
                # Add a lag to the controller input
                ci_fwd, ci_bwd = torch.split(ci, hps.ci_enc_dim[c_idx], dim=2)
                ci_fwd = F.pad(ci_fwd, (0, 0, hps.ci_lag[c_idx], 0, 0, 0))
                ci_bwd = F.pad(ci_bwd, (0, 0, 0, hps.ci_lag[c_idx], 0, 0))
                ...
                fwd_steps = hps.recon_seq_len[c_idx] - hps.encod_seq_len + hps.ic_enc_seq_len
                ci = F.pad(ci, (0, 0, 0, fwd_steps, 0, 0))
                ci_all.append(ci)
            else:
                # Create a placeholder if there's no controller
                ci = torch.zeros(data.shape[0], hps.recon_seq_len[c_idx], 0).to(data.device)
                ci_all.append(ci)

        return ic_mean, ic_std, ci_all
